...
class Agent(object):
    ...
    def updateFitness(self):
        ...
        for s in sensor_info:
            #print(s)
            minSensorDist = min(minSensorDist, s.dist_from_border)
            maxSensorDist = max(maxSensorDist, s.dist_from_border)
        ...
        for id_p in range(len(self.params)/2):
            for s in sensor_info:
                ...
                si = normalise(s.dist_from_border,minSensorDist, maxSensorDist)
        ...
        minSensorDist = minSensorDist / maxSensorDist
        print("minSensorDist : {}".format(minSensorDist))
       # print("vt : {}, vr : {}, minSensorDist : {}".format(vt,vr,minSensorDist))
        self.fitness += (abs(vt)*abs((1-vr))*minSensorDist)
